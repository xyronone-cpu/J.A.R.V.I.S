...
def _press(key: str) -> str:
    ...
    mapped = media_map.get(normalized, normalized)
    try:
        if platform.system() == "Windows" and mapped in {"playpause", "nexttrack", "prevtrack", "stop", "volumemute", "volumeup", "volumedown"}:
            import ctypes
            ...
            ctypes.windll.user32.keybd_event(key_map[mapped], 0, 0, 0)
            ctypes.windll.user32.keybd_event(key_map[mapped], 0, 0x0002, 0)
...
def computer_control(
    ...
    try:
        ...
        if action in {"press_key", "key_press", "media_key"}:
            return _press(params.get("key", "enter"))
