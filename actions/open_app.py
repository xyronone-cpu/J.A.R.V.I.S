...
def _launch_windows(app_name: str) -> bool:
    ...
    try:
        from actions.game_updater import _find_steam_path, _get_steam_games, _steam_exe, _launch_steam_url

        steam_path = _find_steam_path()
        if steam_path:
            requested = app_name.casefold().strip()
            game = next(
                (item for item in _get_steam_games(steam_path)
            ...
            if game is None:
                game = next(
                    (item for item in _get_steam_games(steam_path)
                     if requested in item["name"].casefold()),
                    None,
                )
            if game:
                _launch_steam_url(_steam_exe(steam_path), f"steam://rungameid/{game['id']}")
                time.sleep(2.0)
                return True
    except Exception as e:
        print(f"[open_app] Steam game lookup failed: {e}")
    ...
    try:
        env = os.environ.copy()
        env["MARK_LII_APP_NAME"] = app_name
        result = subprocess.run(
            [
                "powershell.exe", "-NoProfile", "-NonInteractive", "-Command",
                "$name = $env:MARK_LII_APP_NAME; "
                "$app = Get-StartApps | Where-Object { $_.Name -ieq $name } | "
                "Select-Object -First 1; "
                "if (-not $app) { $app = Get-StartApps | "
        ...
        print(f"[open_app] Start Apps lookup failed: {e}")
