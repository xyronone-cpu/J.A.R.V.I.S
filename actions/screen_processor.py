...
def camera_backend_candidates(os_name: str | None = None) -> list[int]:
    """Return candidate OpenCV backends in priority order for this OS."""
    if not _CV2:
        return [0]
    os_name = (os_name or _get_os()).lower()
    if os_name == "windows":
        return [
...
def _cv2_backend() -> int:
    ...
    candidates = camera_backend_candidates()
...
def _capture_camera() -> tuple[bytes, str]:
    ...
    requested_index = _get_camera_index()
    ...
    for index in range(6):
        for backend in backends:
            ...
            cap.release()
            last_error = RuntimeError(f"Camera index {index} returned no frame.")

    if last_error is not None:
        raise last_error
    raise RuntimeError(f"Camera index {requested_index} could not be opened.")
