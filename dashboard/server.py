...
class DashboardServer:
    ...
    def get_url(self) -> str:
        return self._public_url or f"http://{self._ip}:{PORT}"

    def get_manual_url(self) -> str:
        """URL for manual browser entry, public when the tunnel is ready."""
        return self._public_url or f"{self._ip}:{PORT}"

    async def _start_public_tunnel(self) -> None:
        """Expose the local dashboard when cloudflared is installed."""
        executable = shutil.which("cloudflared")
        if not executable:
            print("[Dashboard] Public tunnel unavailable: install cloudflared to enable phone access outside the LAN.")
            return
        ...
        try:
            ...
            while self._tunnel_process and self._tunnel_process.stderr:
                line = await asyncio.to_thread(self._tunnel_process.stderr.readline)
                if not line:
                    break
                match = re.search(r"https://[a-z0-9-]+\.trycloudflare\.com", line)
                if match:
                    self._public_url = match.group(0)
                    print(f"[Dashboard] Public tunnel: {self._public_url}")
                    break
        except Exception as exc:
            print(f"[Dashboard] Public tunnel failed: {exc}")
...
<p>Connecting to JARVIS…</p>
    ...
    async def serve(self) -> None:
        ...
        print(f"[Dashboard] Prueba local: http://127.0.0.1:{PORT}/login")
        tunnel_task = asyncio.create_task(self._start_public_tunnel())
        print("[Dashboard] Press 'Remote Control' in JARVIS UI to get the QR code.")
        try:
            await uvicorn.Server(cfg).serve()
        except OSError as exc:
            print(f"[Dashboard] No se pudo abrir el puerto {PORT}: {exc}")
        finally:
            tunnel_task.cancel()
