...
class _DropCanvas(QWidget):
    ...
    def _paint_idle(self, p, W, H, hover):
        ...
        p.drawText(QRectF(0, cy + 8, W, 16), Qt.AlignmentFlag.AlignCenter,
                   "Suelta el archivo aquí o pulsa para explorar")
        p.setFont(QFont("Courier New", 7))
        p.setPen(QPen(qcol("#1a4a5a"), 1))
        p.drawText(QRectF(0, cy + 24, W, 14), Qt.AlignmentFlag.AlignCenter,
                   "Imágenes · Vídeo · Audio · PDF · Documentos · Código · Datos")
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        lay.addWidget(_lbl("⚙  PERSONALIZAR ASISTENTE", 12, True))
        sep = QFrame(); sep.setFrameShape(QFrame.Shape.HLine)
        sep.setStyleSheet(f"color: {C.BORDER}; margin: 2px 0;")
        lay.addWidget(sep)

        lay.addWidget(_lbl("NOMBRE DEL ASISTENTE", 8, color=C.TEXT_DIM,
                            align=Qt.AlignmentFlag.AlignLeft))
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("TU NOMBRE  (déjalo vacío para usar el tratamiento predeterminado)", 8,
                            color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        self._user_input = QLineEdit(user_name)
        self._user_input.setPlaceholderText("ej.  Tony   (déjalo vacío para automático)")
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("VOZ DEL ASISTENTE", 8, color=C.TEXT_DIM,
                            align=Qt.AlignmentFlag.AlignLeft))
        ...
        lay.addSpacing(4)
        clr_hdr = QHBoxLayout()
        clr_hdr.addWidget(_lbl("COLOR DE LA INTERFAZ  —  arrastra el control", 8,
                               color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft))
        clr_hdr.addStretch()
        df_btn = QPushButton("PREDETERMINADO")
        ...
        self._hex_input.setPlaceholderText("#00d4ff   (color hexadecimal personalizado)")
        ...
        save_btn = QPushButton("▸  APLICAR CAMBIOS")
...
class PluginManagerOverlay(QWidget):
    ...
    def __init__(self, plugins: list[dict], parent=None):
        ...
        hdr = QLabel("🧩  GESTOR DE COMPLEMENTOS")
        ...
        if not plugins:
            empty = QLabel("No se encontraron complementos en /plugins.")
...
class AudioDeviceOverlay(_HudOverlay):
    ...
    def __init__(self, parent=None):
        ...
        hdr = QLabel("🎧  DISPOSITIVOS DE AUDIO")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        lay.addWidget(hdr)
        ...
        self._in_box  = _row("MICRÓFONO — dispositivo con el que JARVIS te escucha",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("ALTAVOCES — dispositivo por el que JARVIS habla",
                             "output", get_output_device())

        note = QLabel("Al aplicar, se reconecta la sesión. Tu conversación se conserva.")
...
class MemoryOverlay(_HudOverlay):
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  LO QUE JARVIS RECUERDA")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        ...
        cap = QLabel(f"{len(rows)} datos guardados — del más reciente al más antiguo. "
                 f"Nada de esto se envía; se guarda en "
                 f"memory/long_term.json en este equipo.")
        ...
        if not rows:
            empty = QLabel("Todavía no hay nada guardado.")
...
class ClipboardPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        icon_lbl = QLabel("◈  PORTAPAPELES DETECTADO")
        ...
        for label, cmd_fmt in [
            ("TRADUCIR", "Traduce este texto al español: {text}"),
            ("RESUMIR", "Resume esto: {text}"),
            ("EXPLICAR", "Explica esto: {text}"),
            ("CORREGIR", "Corrige la gramática y la ortografía: {text}"),
        ]:
            b = QPushButton(label)
            b.setFixedHeight(22)
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto_login_url: str = "",
        ...
        lay.addWidget(_lbl("O introdúcela manualmente:", 7, color=C.TEXT_DIM,
                           align=Qt.AlignmentFlag.AlignLeft))
    ...
    def _tick(self):
        ...
        self._timer_lbl.setText(f"La clave caduca en  {m:02d}:{s:02d}")
        if remaining == 0:
            self._do_close()
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Teléfono conectado — JARVIS está listo")
...
class MainWindow(QMainWindow):
    ...
    def _cam_loop(self) -> None:
        try:
            ...
            if _OS == "Windows":
                ...
                backend_candidates = [cv2.CAP_ANY, 0]

            cap = None
            for idx in range(6):
                for backend in backend_candidates:
                    if backend == 0 and idx != cam_idx and idx != 0:
                        continue
    ...
    def _build_header(self) -> QWidget:
        ...
        self._drawer_btn.setToolTip("Ajustes y controles")
    ...
    def _tick_clock(self):
        ...
        self._date_lbl.setText(
            f"{weekdays[now.tm_wday]} {now.tm_mday} de {months[now.tm_mon - 1]} de {now.tm_year}"
        )
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._file_hint = QLabel("No hay archivo cargado — suéltalo o pulsa arriba para subirlo")
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        sc_btn = QPushButton("⊞  CREAR ACCESO DIRECTO")
        ...
        self._autostart_btn = QPushButton("◉  INICIO AUTOMÁTICO: DESACTIVADO")
        ...
        cust_btn = QPushButton("⚙  PERSONALIZAR ASISTENTE")
        ...
        audio_btn = QPushButton("🎧  DISPOSITIVOS DE AUDIO")
        ...
        plugin_btn = QPushButton("🧩  COMPLEMENTOS")
    ...
    def _build_content_panel(self) -> QWidget:
        ...
        self._content_title_lbl = QLabel("INFORME")
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    command, _ = winreg.QueryValueEx(key, "JARVIS_AI")
                    expected_exe = Path(sys.executable).resolve()
                    if getattr(sys, "frozen", False):
                        return str(expected_exe) in command
                    expected_script = str((Path(__file__).resolve().parent / "main.py").resolve())
                    return str(expected_exe.parent / "pythonw.exe") in command and expected_script in command
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    ...
                    if getattr(sys, "frozen", False):
                        ...
                        command = f'"{exe.resolve()}" "{Path(script).resolve()}"'
    ...
    def _update_autostart_btn(self, enabled: bool):
        if not hasattr(self, '_autostart_btn'):
            return
        if enabled:
            self._autostart_btn.setText("◉  INICIO AUTOMÁTICO: ACTIVADO")
            ...
            self._autostart_btn.setText("◉  INICIO AUTOMÁTICO: DESACTIVADO")
            self._autostart_btn.setStyleSheet(f"""
                QPushButton {{
                    background: transparent; color: {C.TEXT_DIM};
                    border: 1px solid {C.BORDER}; border-radius: 3px;
    ...
    def _update_brief_btn(self, enabled: bool):
        if not hasattr(self, '_brief_btn'):
            return
        if enabled:
            self._brief_btn.setText("☀  INFORME MATUTINO: ACTIVADO")
            ...
            self._brief_btn.setText("☀  INFORME MATUTINO: DESACTIVADO")
            self._brief_btn.setStyleSheet(f"""
                QPushButton {{
                    background: transparent; color: {C.TEXT_DIM};
                    border: 1px solid {C.BORDER}; border-radius: 3px;
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        if display in ("JARVIS", "J.A.R.V.I.S"):
            self._sub_lbl.setText("Solo un sistema bastante inteligente")
        else:
            self._sub_lbl.setText("Asistente personal de IA")
        ...
        try:
            ...
            self._log.append_log(f"SYS: Identidad actualizada — {display}")
            if color_changed:
                self._log.append_log(f"SYS: Color de interfaz aplicado — {ui_color}")
            if voice_changed:
                self._log.append_log(f"SYS: Voz configurada — {voice}")
        except Exception as e:
            self._log.append_log(f"ERR: Falló el guardado de configuración — {e}")
    ...
    def _on_audio_devices_applied(self):
        self._log.append_log("SYS: Dispositivos de audio actualizados.")
